...
def run_tune(config, return_analysis=False):
    ...
    resources_per_trial = {'cpu': config.get('cpus_per_trial', 4), 'gpu': config.get('gpus_per_trial', 1 if torch.cuda.is_available() else 0)},
    scheduler = scheduler,
    stop = {'training_iteration': config.get('max_t', 500)},
